...
This script is designed to be called from plugin hooks.
...
notification (using terminal-notifier, if available) and a text-to-speech
(TTS) announcement.

Desktop notifications require terminal-notifier (macOS only).
TTS works on any platform with uv and Kokoro server running.
"""
...
# --- CONFIGURATION ---
# Path to TTS script relative to this file's location
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
PLUGIN_ROOT = os.path.dirname(SCRIPT_DIR)
TTS_SCRIPT_PATH: str = os.path.join(PLUGIN_ROOT, "python", "claude_tts.py")
...
def send_notification(title: str, subtitle: str, message: str, tts_message: Optional[str] = None) -> None:
    ...
    """
    # 1. Try desktop notification (fails silently if terminal-notifier not available)
    try:
        ...
        # Desktop notification unavailable - continue with TTS
        pass

    # 2. TTS announcement (this should always work if Kokoro is running)
    try:
        ...
        subprocess.run(tts_cmd, check=True, timeout=7,
                      cwd=os.path.join(PLUGIN_ROOT, "python"))
    except (subprocess.TimeoutExpired, subprocess.SubprocessError, FileNotFoundError):
        # TTS failed - fail silently
        pass
    except Exception:
